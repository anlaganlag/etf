...
# LONG PERIOD CONFIGS
LONG_PERIOD_CONFIGS = {
    'Hybrid': {5: 50, 10: 50, 20: 70, 60: 100, 120: 80, 250: 50},
    'Inst': {20: 50, 60: 100, 120: 100, 250: 80},
    'Ancient': {120: 100, 250: 100, 500: 50}
}

def init(context):
    config_key = os.getenv('LONG_MODE', 'Hybrid')
    context.periods_rule = LONG_PERIOD_CONFIGS[config_key]
    ...
    # Load price matrix - Need enough history for 500 days
    price_data = {}
...
def on_bar(context, bars):
    ...
    if (context.days_count - 1) % context.T == 0:
        ...
        if len(history_prices) < 251: return # We need enough for ranking
...
def on_backtest_finished(context, indicator):
    print(f"LONG_RESULT|{context.s_name}|{indicator.get('pnl_ratio',0)*100:.2f}|{indicator.get('max_drawdown',0)*100:.2f}|{indicator.get('sharp_ratio',0):.2f}")

if __name__ == '__main__':
    token = os.getenv('MY_QUANT_TGM_TOKEN')
    for m in ['Hybrid', 'Inst', 'Ancient']:
        os.environ['LONG_MODE'] = m
        print(f">>> Testing Long Period Mode: {m}...")
        run(strategy_id='fe474f75-fa8e-11f0-b097-00ffda9d6e63',
            filename='test_long_period.py',
            mode=MODE_BACKTEST,
            token=token,
