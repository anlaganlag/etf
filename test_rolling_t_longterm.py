# coding=utf-8
"""
Rolling策略T值优化测试 - 长周期验证
对比 T=6,7,8,10,12,14 在2021-2026全周期的表现
"""
...
# Reuse the same Tranche and functions from previous test
class Tranche:
...
def backtest_rolling_strategy(T, top_n=5, start_date='2021-12-01', end_date='2026-01-27'):
    print(f"\n{'='*60}")
    print(f"Testing T={T}")
    print(f"{'='*60}")
...
def main():
    print("\n" + "="*80)
    print("Rolling Strategy T-Value Optimization - Long-term Test")
    print("测试区间: 2021-12-01 至 2026-01-27 (全周期)")
    print("="*80)
    ...
    summary_df = pd.DataFrame([{
        ...
        '年化收益(%)': f"{r['Annualized_Return_Pct']:.2f}",
    ...
    output_path = os.path.join(config.BASE_DIR, 'output', 'data', 'rolling_t_longterm_comparison.csv')
    summary_df.to_csv(output_path, index=False, encoding='utf-8-sig')
    print(f"\n结果已保存至: {output_path}")

    best_return = max(results, key=lambda x: x['Final_Return_Pct'])
    best_sharpe = max(results, key=lambda x: x['Sharpe_Ratio'])

    print(f"\n🏆 最高收益: T={best_return['T']} ({best_return['Final_Return_Pct']:.2f}%)")
    print(f"🏆 最佳夏普: T={best_sharpe['T']} ({best_sharpe['Sharpe_Ratio']:.2f})")

    # Risk-adjusted ranking
    print("\n📊 风险调整排名 (收益/回撤比):")
    for r in sorted(results, key=lambda x: x['Final_Return_Pct']/abs(x['Max_Drawdown_Pct']), reverse=True):
        ratio = r['Final_Return_Pct'] / abs(r['Max_Drawdown_Pct'])
        print(f"  T={r['T']:2d}: {ratio:.2f} (收益{r['Final_Return_Pct']:.1f}% / 回撤{r['Max_Drawdown_Pct']:.1f}%)")
