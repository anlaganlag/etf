...
class RollingPortfolioManager:
        ...
        if os.path.exists(self.state_path):
            try:
                with open(self.state_path, 'r', encoding='utf-8') as f:
                    ...
                    self.days_count = data.get("days_count", 0) # Persist rotation state
    ...
    def reconcile_with_broker(self, real_positions):
        ...
        for sym, v_qty in virtual_map.items():
            ...
            diff = v_qty - r_qty
            if diff > 0: # Virtual > Real -> Phantom
                print(f"⚠️ Reconcile: Found {diff} phantom shares of {sym}. Fixing...")
                remaining = diff
                for t in self.tranches:
                    if sym in t.holdings:
                        ...
                        if remove_qty > 0:
                            ...
                            if t.holdings[sym] == 0: del t.holdings[sym]
                            # Refund Cash
                            if sym in t.pos_records:
                                t.cash += remove_qty * t.pos_records[sym]['entry_price']
                                if sym not in t.holdings: del t.pos_records[sym]
...
def algo(context):
    ...
    if context.mode == MODE_LIVE:
        try:
            ...
            today_data = {t['symbol']: t['price'] for t in ticks if t['price'] > 0}
            if today_data:
                today_dt = current_dt.replace(hour=0, minute=0, second=0, microsecond=0)
                today_s = pd.Series(today_data, name=today_dt)
                if today_dt in context.prices_df.index:
                    context.prices_df.loc[today_dt, today_s.index] = today_s
                else:
                    context.prices_df = pd.concat([context.prices_df, today_s.to_frame().T])
                context.prices_df.sort_index(inplace=True)
                print(f"☁️ Live: Injected {len(today_data)} ticks.")
        except Exception as e:
    ...
    try:
        ...
        context.rpm.reconcile_with_broker(real_pos)
    except Exception as e: print(f"⚠️ Reconcile Fail: {e}")

    # === 3. Strategy Logic ===
    history = context.prices_df[context.prices_df.index <= current_dt]
    if history.empty: return
    price_map = history.iloc[-1].to_dict()
    ...
    if ranking_df is not None and not active_tranche.guard_triggered_today:
        targets = []
        if MAX_PER_THEME > 0: # Theme Filter
            theme_count = {}
            for c, r in ranking_df.iterrows():
                if theme_count.get(r['theme'], 0) < MAX_PER_THEME:
                    targets.append(c)
        ...
        if targets:
            ...
            if DYNAMIC_POSITION:
                usable_cash *= get_market_regime(context, current_dt)
            
            # Unequal Weights (Top 3 x2)
            weights = [2 if i<3 else 1 for i in range(len(targets))]
            unit = usable_cash / sum(weights)
            
            for i, sym in enumerate(targets):
                active_tranche.buy(sym, unit * weights[i], price_map.get(sym,0))
    ...
    global_tgt = {}
    for t in context.rpm.tranches:
        for s, n in t.holdings.items(): global_tgt[s] = global_tgt.get(s, 0) + n

    # Minimal Sell Logic (Safe T+1)
    # Iterate ALL broker positions to catch sells (both rotation and guards)
    for p in context.account().positions():
        tgt = global_tgt.get(p.symbol, 0)
        diff = p.amount - tgt
        if diff > 0 and p.available > 0:
            vol = int(min(diff, p.available))
            order_volume(symbol=p.symbol, volume=vol, side=OrderSide_Sell, order_type=OrderType_Market, position_effect=PositionEffect_Close)
            print(f"📉 Sell {p.symbol}: {vol}", flush=True)

    # Buy Logic
    real_map = {p.symbol: p.amount for p in context.account().positions()} 
    for sym, tgt in global_tgt.items():
        curr = real_map.get(sym, 0)
        if curr < tgt:
            order_target_volume(symbol=sym, volume=int(tgt), order_type=OrderType_Market, position_side=PositionSide_Long)

    # Sim Stats
    context.rpm.nav_history.append(sum(t.total_value for t in context.rpm.tranches))
...
if __name__ == '__main__':
    # RUN_MODE = 'LIVE' # Uncomment to go live
    RUN_MODE = 'BACKTEST'
    ...
    if RUN_MODE == 'LIVE':
        ...
        run(strategy_id=STRATEGY_ID, filename='gm_strategy_rolling0_c.py', mode=MODE_LIVE, token=os.getenv('MY_QUANT_TGM_TOKEN'))
    else:
        print("📉 Starting BACKTEST...")
        run(strategy_id=STRATEGY_ID, filename='gm_strategy_rolling0_c.py', mode=MODE_BACKTEST,
