...
def init(context):
    # 1. Ultimate (Equal Weight) Parameters
    context.T = 14                  
    context.top_n = 10              
...
def on_bar(context, bars):
    ...
    if (context.days_count - 1) % context.T == 0:
        ...
        rebalance_equal(context, current_dt)

def rebalance_equal(context, current_dt):
    current_date_str = current_dt.strftime('%Y-%m-%d')
    print(f"\n--- [{current_date_str}] Equal Weight Rebalancing (Day {context.days_count}) ---")
    ...
    if valid_scores.empty:
        ...
        for code, row in sorted_df.iterrows():
            ...
            if len(target_list) >= context.top_n:
                break
    
    # 5. Execution (EQUAL WEIGHT)
    ...
    if target_list:
        ...
        weight = (1.0 / len(target_list)) * context.target_percent * market_exposure
...
def on_backtest_finished(context, indicator):
    print("\n" + "="*50)
    print("ULTIMATE EQUAL WEIGHT REPORT")
    print("="*50)
