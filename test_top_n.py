...
def init(context):
    # Get top_n from environment variable
    context.top_n = int(os.getenv('TEST_TOP_N', 10))
    context.T = 14
    context.min_score = 0  # NO THRESHOLD - pure ranking
...
def on_bar(context, bars):
    ...
    if (context.days_count - 1) % context.T == 0:
        ...
        # Filter only whitelist (NO min_score filter)
...
def on_backtest_finished(context, indicator):
    print(f"TOP_N_RESULT|{context.top_n}|{indicator.get('pnl_ratio',0)*100:.2f}|{indicator.get('max_drawdown',0)*100:.2f}|{indicator.get('sharp_ratio',0):.2f}")

if __name__ == '__main__':
    token = os.getenv('MY_QUANT_TGM_TOKEN')
    
    print("=" * 60)
    print("Testing Top N Holdings (No Min Score) - Full Period")
    print("=" * 60)
    
    for n in range(2, 11):
        os.environ['TEST_TOP_N'] = str(n)
        print(f"\n>>> Testing Top {n}...")
        run(strategy_id='fe474f75-fa8e-11f0-b097-00ffda9d6e63',
            filename='test_top_n.py',
            mode=MODE_BACKTEST,
            token=token,
