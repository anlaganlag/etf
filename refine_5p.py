...
# THE THREE ARCHETYPES OF 5P
REFINEMENT_CONFIGS = {
    'Base': {1: 100, 3: 70, 5: 50, 10: 30, 20: 20},      # Current
    'Smooth': {1: 60, 3: 80, 5: 100, 10: 80, 20: 60},    # Focus on trend durability
    'Golden': {1: 80, 3: 80, 5: 80, 10: 80, 20: 80}      # Pure consensus
}

def init(context):
    config_key = os.getenv('REFINE_MODE', 'Base')
    context.periods_rule = REFINEMENT_CONFIGS[config_key]
...
def on_backtest_finished(context, indicator):
    print(f"REFINE_RESULT|{context.s_name}|{indicator.get('pnl_ratio',0)*100:.2f}|{indicator.get('max_drawdown',0)*100:.2f}|{indicator.get('sharp_ratio',0):.2f}")

if __name__ == '__main__':
    token = os.getenv('MY_QUANT_TGM_TOKEN')
    for m in ['Base', 'Smooth', 'Golden']:
        os.environ['REFINE_MODE'] = m
        print(f">>> Testing Refinement: {m}...")
        run(strategy_id='fe474f75-fa8e-11f0-b097-00ffda9d6e63',
            filename='refine_5p.py',
            mode=MODE_BACKTEST,
            token=token,
            backtest_start_time='2025-01-23 09:00:00', # 12 months for speed
