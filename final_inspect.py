...
def init(context):
    ...
    # Data Loader (cached in init class to share across runs)
...
def on_backtest_finished(context, indicator):
    print(f"RES|{os.getenv('TF')}|{indicator.get('pnl_ratio', 0)*100:.2f}|{indicator.get('max_drawdown', 0)*100:.2f}|{indicator.get('sharp_ratio',0):.2f}")
...
if __name__ == '__main__':
    ...
    print("TF | PNL | MDD | SHARP")
    for tf_name, start in tfs.items():
        os.environ['TF'] = tf_name
        run(strategy_id='fe474f75-fa8e-11f0-b097-00ffda9d6e63', filename='final_inspect.py', mode=MODE_BACKTEST, token=token,
            backtest_start_time=f'{start} 09:00:00', backtest_end_time='2026-01-23 16:00:00', backtest_adjust=ADJUST_PREV, backtest_initial_cash=1000000)
