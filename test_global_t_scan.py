...
def run_global_t_scan():
    print("=== Global T-Period Scan (Post-924, 2024-11-01 to Present) ===")
    config.ensure_dirs()
    ...
    for T in range(6, 31):
        ...
        for i in range(len(valid_dates) - 1):
            ...
            if curr_holdings:
                val *= (1 + daily_rets.loc[valid_dates[i], curr_holdings].mean())
        
        ret = (val - 1.0) * 100
        results.append({'T': T, 'Return': ret})
        print(f"  T={T:<2}: {ret:>6.1f}%")
