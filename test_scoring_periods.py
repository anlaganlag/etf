...
# Two scoring configurations to test
SCORING_CONFIGS = {
    'short_5periods': {
        'name': 'Short-Term (5 Periods: 1,3,5,10,20)',
        'periods': {1: 100, 3: 70, 5: 50, 10: 30, 20: 20}
    },
    'full_8periods': {
        'name': 'Full Spectrum (8 Periods: +60,120,250)',
        'periods': {1: 100, 3: 70, 5: 50, 10: 30, 20: 20, 60: 15, 120: 10, 250: 5}
    }
}

def init(context):
    config_key = os.getenv('SCORING_CONFIG', 'full_8periods')
    context.scoring_config = SCORING_CONFIGS[config_key]
    context.periods_rule = context.scoring_config['periods']
    
    context.T = 14
    context.top_n = 10
    context.min_score = 0  # No threshold for fair comparison
    ...
    for f in os.listdir(DATA_CACHE_DIR):
        if f.endswith('.csv') and ('SHSE' in f or 'SZSE' in f):
    ...
    if (context.days_count - 1) % context.T == 0:
        ...
        if not valid.empty:
            ...
            if selected:
                ...
                avg_s = sum_s / len(selected) if len(selected) > 0 else 1
...
def on_backtest_finished(context, indicator):
    config_name = context.scoring_config['name']
    print(f"SCORE_CONFIG_RESULT|{config_name}|{indicator.get('pnl_ratio',0)*100:.2f}|{indicator.get('max_drawdown',0)*100:.2f}|{indicator.get('sharp_ratio',0):.2f}")

if __name__ == '__main__':
    token = os.getenv('MY_QUANT_TGM_TOKEN')
    
    print("=" * 70)
    print("Comparing Scoring Period Configurations")
    print("=" * 70)
    
    for config_key in ['short_5periods', 'full_8periods']:
        os.environ['SCORING_CONFIG'] = config_key
        config_name = SCORING_CONFIGS[config_key]['name']
        print(f"\n>>> Testing: {config_name}")
        run(strategy_id='fe474f75-fa8e-11f0-b097-00ffda9d6e63',
            filename='test_scoring_periods.py',
            mode=MODE_BACKTEST,
            token=token,
