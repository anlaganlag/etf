...
# --- Simulation Settings ---
# Run (5, 13) first to verify baseline
TOP_N_RANGE = [5]
T_RANGE = [13]
...
def simulate(top_n, t_period, prices_df, whitelist, theme_map):
    ...
    for i in range(start_idx, end_idx + 1):
        ...
        # Look-ahead buy/sell at today's price but in the 'on_bar' context it might be different
        # Actually, let's try WITHOUT look-ahead first but ensure BUY/SELL happens at the bar price.
        ...
        if not active_tranche.guard_triggered_today:
            ...
            if ranking_df is not None:
                ...
                if targets:
                    ...
                    for sym in targets:
                        active_tranche.buy(sym, per_amt, price_map_today.get(sym, 0))
        
        total_val = sum(t.total_value for t in tranches)
...
def main():
    ...
    for f in files:
        ...
        if '.' not in code: code = ('SHSE.' if code.startswith('sh') else 'SZSE.') + code[2:]
    ...
    res = simulate(5, 13, prices_df, whitelist, theme_map)
    print(f"Result (5, 13): {res:.2f}%")
